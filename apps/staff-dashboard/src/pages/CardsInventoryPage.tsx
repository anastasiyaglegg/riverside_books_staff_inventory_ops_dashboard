...
export function CardsInventoryPage() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const cards = await api.get<Card[]>("/cards");
      setItems([...cards].sort((a, b) => a.title.localeCompare(b.title)));
    } catch (err) {
  ...
  async function saveQuantity(item: Card) {
    ...
    setError(null);
    try {
      const updated = await api.patch<Card>(`/cards/${item.id}`, {
        quantityOnHand,
      });
